            ...
            <form id="myForm">
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="pemilik" id="pemilik" class="form-control <?= $validation->hasError('pemilik') ? 'is-invalid' : '' ?>" required <?= $is_register > 0 ? 'disabled' : ''?>>
                            <option value="" selected disabled>- Pilih -</option>
                            <?php foreach ($pemilik as $item) : ?>
                                <option value="<?= $item->id_user ?>"<?= $is_register > 0 ? ($pendaftaran[0]->id_customer == $item->id_user ? 'selected' : '') : '' ?> <?= set_select('pemilik', $item->id_user) ?> ><?= $item->id_user . ' / ' . $item->nama_lengkap ?></option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <!-- <?php if (count($detail) > 0) { ?>
                        <div class="form-group">
                            ...
                            <select name="pemilik" id="pemilik" class="form-control <?= $validation->hasError('pemilik') ? 'is-invalid' : '' ?>" required>
                                <option value="" selected disabled>- Pilih -</option>
                                <?php foreach ($pemilik as $item) : ?>
                                    <option value="<?= $item->id_user ?>" <?= set_select('pemilik', $item->id_user) ?>><?= $item->id_user . ' / ' . $item->nama_lengkap ?></option>
                                <?php endforeach; ?>
                            </select>
                            <?php if ($validation->getError('pemilik')) { ?>
                                <div class="invalid-feedback">
